...
DCOS_OS_REQUIREMENTS = <<-SHELL
  ...
  #yum install --assumeyes --tolerant --quiet vim strace perf bash-completion
...
DCOS_BOOT_PROVISION = <<-SHELL
  ...
  sudo -i -u vagrant sh -c 'mkdir -p dcos && cd dcos && virtualenv --quiet . && source bin/activate && pip install --quiet --upgrade pip dcoscli httpie'
